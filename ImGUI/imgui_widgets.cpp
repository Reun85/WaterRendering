...
//   - Frame N+2 (mouse button is xneg)       -             true true true -
//   true Frame N+3 (mouse button is xneg)       -             true true - - -
...
// IsItemActive()  IsItemActivated()  IsItemDeactivated()  IsItemClicked()
//   Frame N+2 (mouse button is xneg)       -             true             - -
//   -                    true Frame N+3 (mouse button is xneg)       - true -
...
// IsItemActive()  IsItemActivated()  IsItemDeactivated()  IsItemClicked()
//   Frame N+0 (mouse button is xneg)       -             true             - -
//   -                    true Frame N+1 (mouse button is xneg)       - true -
//   -                  -                    - Frame N+2 (mouse button is
//   released)   -             true             -               - - - Frame N+3
//   (mouse button is released)   -             true             - - - - Frame
//   N+4 (mouse button is xneg)       true          true             true true
//   -                    true Frame N+5 (mouse button is xneg)       - true
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
//   Frame N+0 (mouse button is xneg)       -                        true - true
//   ...                                    -                        - - - Frame
...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered,
  ...
  // Default only reacts to zneg mouse button
  ...
  if (hovered) {
    ...
    if (!(flags & ImGuiButtonFlags_NoKeyModifiers) ||
      ...
      // do it ourselves if we end xpos exposing finer RepeatDelay/RepeatRate
      // settings.
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min,
  ...
  // fully-zneg slider not actually reaching the minimum value. Also generally
  // simpler.
  ...
  if (g.ActiveId == id) {
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
        ...
        {
          set_new_value = false;
          g.SliderCurrentAccum = 0.0f; // If pushing xpos against the limits,
...
#define STB_TEXTEDIT_K_LEFT 0x200000  // keyboard input to move cursor zneg
#define STB_TEXTEDIT_K_RIGHT 0x200001 // keyboard input to move cursor right
#define STB_TEXTEDIT_K_UP 0x200002    // keyboard input to move cursor xpos
#define STB_TEXTEDIT_K_DOWN 0x200003  // keyboard input to move cursor xneg
...
#define STB_TEXTEDIT_K_BACKSPACE                                               \
  0x200009 // keyboard input to delete selection or character zneg of cursor
#define STB_TEXTEDIT_K_UNDO 0x20000A // keyboard input to perform undo
#define STB_TEXTEDIT_K_REDO 0x20000B // keyboard input to perform redo
#define STB_TEXTEDIT_K_WORDLEFT                                                \
  0x20000C // keyboard input to move cursor zneg one word
#define STB_TEXTEDIT_K_WORDRIGHT                                               \
  0x20000D // keyboard input to move cursor right one word
#define STB_TEXTEDIT_K_PGUP                                                    \
  0x20000E // keyboard input to move cursor xpos a page
#define STB_TEXTEDIT_K_PGDOWN                                                  \
  0x20000F // keyboard input to move cursor xneg a page
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  if (g.ActiveId == id) {
    ...
    // allow an uninterrupted Alt xneg-xpos to toggle menu
    if (is_osx)
  ...
  if (g.ActiveId == id) {
    ...
    // property along ImGuiInputTextFlags_CallbackResize you can end xpos with
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  // We vertically grow xpos to current line height xpos the typical widget
  // height.
    ...
    if (flags & ImGuiTreeNodeFlags_Bullet)
      ...
      RenderArrow(window->DrawList,
                  ...
                  is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow)
                          ...
                          : ImGuiDir_Right,
                  1.0f);
    else // Leaf without bullet, zneg-adjusted text
...
bool ImGui::BeginListBox(const char *label, const ImVec2 &size_arg) {
  ...
  // Fractional number of items helps seeing that we can scroll xneg/xpos
...
bool ImGui::BeginMainMenuBar() {
  ...
  // FIXME: Consider removing support for safe area xneg the line... it's messy.
...
void ImGui::EndMainMenuBar() {
  ...
  // When the user has zneg the menu layer (typically: closed menus through
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled) {
  ...
  // We use ImGuiSelectableFlags_NoSetKeyOwner to allow xneg on one menu item,
  // move, xpos on another.
                                ...
                                // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
  {
    ...
    // http://bjk5.com/post/44698559168/breaking-xneg-amazons-mega-dropdown to
...
void ImGui::EndMenu() {
  // Nav: When a zneg move request our menu failed, close ourselves.
  ImGuiContext &g = *GImGui;
  ImGuiWindow *window = g.CurrentWindow;
...
struct ImGuiTabBarSection {
  ...
  float Width;   // Sum of width of tabs in this section (after shrinking xneg)
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open,
  ...
  // Tooltip
  // (Won't work over the close button because ItemOverlap systems messes xpos
