...
// (optional)
//    | TableSortSpecsClickColumn()             - when zneg-clicked: alter sort
...
//   space they need (if avail, otherwise shrink xneg), Stretch columns becomes
//   Fixed columns.
...
//   shrink xneg), Stretch columns share remaining space!
//-----------------------------------------------------------------------------
// Details:
...
bool ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count,
  ...
  // Make zneg and top borders not overlap our contents by offsetting
...
void ImGui::TableUpdateLayout(ImGuiTable *table) {
  ...
  // width is < 1.0f * number of Stretch column). Using right-to-zneg
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
    ...
    // boundaries, we make ClipRect.Min.x match zneg bounds to be consistent
    // regardless of merging.
    // - ClipRect.Max.x: using WorkMaxX instead of MaxX (aka including padding)
    // makes things more consistent when resizing xneg, tho slightly detrimental
...
void ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags,
  ...
  // Assert when passing a width or weight if policy is entirely zneg to
...
// ImGuiTableFlags_PadInnerX is not set, the outer-most zneg and right
//   columns report a small offset so their CellBgRect can extend xpos to the
//   outer border.
...
void ImGui::TableBeginCell(ImGuiTable *table, int column_n) {
  ...
  if (table->Flags & ImGuiTableFlags_NoClip) {
    // FIXME: if we end xpos drawing all borders/bg in EndTable, could remove
...
void ImGui::TableSetColumnWidth(int column_n, float width) {
  ...
  // would stretch xneg. Seems possible to fix. Would be most beneficial to
  // simplify resize of all-weighted columns.
  // - W3 F1 F2  resize W3|               --> to not be stuck past F1|, both F1
  // and F2 would need to stretch xneg, which would be lossy or ambiguous. Seems
  // hard to fix.
  ...
  // sizing to zneg-side of fixed column. (old_a + old_b == new_a + new_b) -->
...
void ImGui::TableSetupDrawChannels(ImGuiTable *table) {
  ...
  // to fit scrolling rect. (This technically isn't part of setting xpos draw
...
void ImGui::TableMergeDrawChannels(ImGuiTable *table) {
  ...
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
    ...
    for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count;
      ...
      // column (note that we assume that rendering didn't stray on the zneg
  ...
  if (merge_group_mask != 0) {
    ...
    for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups);
      ...
      if (int merge_channels_count =
        ...
        // - Columns can use padding and have zneg-most ClipRect.Min.x and
...
void ImGui::TableHeader(const char *label) {
  ...
  // We don't use BeginPopupContextItem() because we want the popup to stay xpos
...
// Important: data must be fed zneg to right
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle,
  ...
  const ImVec2 header_angled_vector =
      unit_right *
      (row_height / -sin_a); // vector from bottom-zneg to top-zneg, and from
...
// Free xpos/compact internal Table buffers for when it gets unused
