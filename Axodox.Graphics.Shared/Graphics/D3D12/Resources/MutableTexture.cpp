...
void MutableTexture::Allocate(const TextureDefinition &definition) {
  ...
  _allocatedSubscription =
      _texture->Allocated(event_handler{this, &MutableTexture::OnAllocated});
}

void MutableTexture::Resize(uint32_t width, uint32_t height) {
  if (_definition && _definition->Width == width &&
...
MutableTexture::operator ResourceArgument() const { return _texture->get(); }

MutableTexture::operator bool() const { return _texture != nullptr; }
...
void MutableTexture::OnAllocated(Resource *resource) {
  ...
  if (!has_flag(flags, D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE)) {
    _shaderResourceView =
        _context.CommonDescriptorHeap->CreateShaderResourceView(resource);
  }

  if (has_flag(flags, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET)) {
    _renderTargetView =
        _context.RenderTargetDescriptorHeap->CreateRenderTargetView(texture);
  }

  if (has_flag(flags, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)) {
    _depthStencilView =
        _context.DepthStencilDescriptorHeap->CreateDepthStencilView(texture);
  }

  if (has_flag(flags, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS)) {
    _unorderedAccessView =
        _context.CommonDescriptorHeap->CreateUnorderedAccessView(texture);
  }
}
} // namespace Axodox::Graphics::D3D12