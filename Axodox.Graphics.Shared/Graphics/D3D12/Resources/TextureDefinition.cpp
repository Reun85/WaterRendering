...
TextureHeader::TextureHeader()
    : PixelFormat(Format::Unknown), Width(0), Height(0), Depth(0), MipCount(1),
      ArraySize(0) {}
...
TextureHeader::TextureHeader(const D3D12_RESOURCE_DESC &description)
    : PixelFormat(Format(description.Format)),
      ...
      Height(description.Dimension >= D3D12_RESOURCE_DIMENSION_TEXTURE2D
                 ? description.Height
                 : 0u),
      Depth(description.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE3D
                ? description.DepthOrArraySize
                : 0u),
      ArraySize(description.Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE3D
                    ? description.DepthOrArraySize
                    : 0u),
      MipCount(description.MipLevels) {}

TextureHeader::TextureHeader(Format format, uint32_t width, uint32_t height,
                             uint32_t arraySize)
    : PixelFormat(format), Width(width), Height(height), Depth(0), MipCount(1),
      ArraySize(arraySize) {}
...
TextureDefinition::TextureDefinition(Format format, uint32_t width,
                                     ...
                                     TextureFlags flags)
    : TextureHeader(format, width, height, arraySize), SampleCount(1),
      SampleQuality(0), Flags(flags) {}
...
bool TextureDefinition::AreSizeCompatible(const TextureDefinition &a,
                                          const TextureDefinition &b) {
  return a.Width == b.Width && a.Height == b.Height && a.Depth == b.Depth &&
         a.ArraySize == b.ArraySize && a.MipCount == b.MipCount &&
         a.SampleCount == b.SampleCount && a.SampleQuality == b.SampleQuality;
}

TextureDefinition::TextureDefinition(const D3D12_RESOURCE_DESC &description)
    : TextureHeader(description), SampleCount(description.SampleDesc.Count),
      SampleQuality(description.SampleDesc.Quality),
      Flags(TextureFlags(description.Flags)) {}

ResourceType TextureDefinition::Type() const { return ResourceType::Texture; }
...
TextureDefinition::operator D3D12_RESOURCE_DESC() const {
  ...
  return D3D12_RESOURCE_DESC{
      ...
      .DepthOrArraySize = uint16_t(
          max(1u, dimension == D3D12_RESOURCE_DIMENSION_TEXTURE3D ? Depth
                                                                  : ArraySize)),
      .MipLevels = MipCount,
...
TextureData::TextureData() : _header({}) {}

TextureData::TextureData(Format format, uint32_t width, uint32_t height,
                         uint16_t arraySize)
...
TextureData::operator bool() const { return !_buffer.empty(); }
...
void TextureData::UpdateLayout() {
  ...
  for (auto mip = 0; mip < _header.MipCount; mip++) {
    ...
    if (rowPitch % 8 != 0)
      rowPitch += 8 - rowPitch % 8;
    rowPitch >>= 3;

    auto depthPitch = rowPitch * max(1u, _header.Height >> mip);

    auto mipPitch =
        max(1u, _header.Depth > 0 ? _header.Depth >> mip : _header.ArraySize) *
        depthPitch;

    TextureLayout layout{.Offset = offset,
                         .RowPitch = rowPitch,
...
TextureData TextureData::FromBuffer(std::span<const uint8_t> buffer) {
  ...
  com_ptr<IWICBitmapSource> wicBitmap;
  {
    ...
    check_hresult(wicFactory->CreateDecoderFromStream(
        stream.get(), NULL, WICDecodeMetadataCacheOnDemand,
        wicBitmapDecoder.put()));

    check_hresult(wicBitmapDecoder->GetFrame(
        0, reinterpret_cast<IWICBitmapFrameDecode **>(wicBitmap.put())));
  }
  ...
  // Covert data to target format
  {
    ...
    check_hresult(wicFormatConverter->Initialize(
        wicBitmap.get(), GUID_WICPixelFormat32bppBGRA, WICBitmapDitherTypeNone,
  ...
  check_hresult(
      wicBitmap->CopyPixels(&rect, stride, uint32_t(span.size()), span.data()));
...
TextureDefinition TextureData::Definition() const { return Header(); }
...
void TextureData::CopyToResource(ID3D12Resource *resource) const {
  ...
  device->GetCopyableFootprints(&description, 0, uint32_t(layouts.size()), 0ull,
                                layouts.data(), nullptr, nullptr, nullptr);

  uint8_t *pBuffer;
  check_hresult(
      resource->Map(0u, &EmptyRange, reinterpret_cast<void **>(&pBuffer)));
  ...
  for (auto mip = 0u; mip < description.MipLevels; mip++) {
    for (auto slice = 0u; slice < description.DepthOrArraySize; slice++) {
      auto subresourceIndex = GetSubresourceIndex(
          mip, slice, 0u, description.MipLevels, description.DepthOrArraySize);
...
const TextureHeader &TextureData::Header() const { return _header; }
...
std::span<uint8_t> TextureData::AsRawSpan(uint32_t *stride, uint32_t slice,
                                          uint32_t mip) {
  if (_buffer.empty())
    return {};
  if (mip > _header.MipCount)
    throw out_of_range("Mip index is out of range!");
  if (slice > max(_header.Depth, _header.ArraySize))
    throw out_of_range("Slice index is out of range!");

  auto &mipLayout = _mipLayouts[mip];
  if (stride)
    *stride = uint32_t(mipLayout.RowPitch);

  return {_buffer.data() + mipLayout.Offset + mipLayout.DepthPitch * slice,
          mipLayout.DepthPitch};
}

std::span<const uint8_t>
TextureData::AsRawSpan(uint32_t *stride, uint32_t slice, uint32_t mip) const {
