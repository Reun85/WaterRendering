...
template <typename T> class RootConstant : public RootParameter {
public:
  RootConstant(RootSignatureMask *owner, InputSlot slot,
               ShaderVisibility visibility = ShaderVisibility::All)
      : RootParameter(owner, visibility), _slot(slot) {}
  ...
  void operator=(const T &value) {
    ...
    case RootSignatureUsage::Graphics:
      allocator->SetGraphicsRoot32BitConstants(Index, Size(),
                                               AsBuffer(value).data(), 0);
      break;
    case RootSignatureUsage::Compute:
      allocator->SetComputeRoot32BitConstants(Index, Size(),
                                              AsBuffer(value).data(), 0);
      break;
    default:
...
template <RootDescriptorType Type> class RootDescriptor : public RootParameter {
public:
  RootDescriptor(RootSignatureMask *owner, InputSlot slot,
                 ShaderVisibility visibility = ShaderVisibility::All)
      : RootParameter(owner, visibility), _slot(slot) {}
...
template <uint32_t Size> class RootDescriptorTable : public RootParameter {
public:
  RootDescriptorTable(RootSignatureMask *owner,
  ...
  void operator=(GpuVirtualAddress reference) {
    ...
    switch (Context()->Usage) {
    case RootSignatureUsage::Graphics:
      allocator->SetGraphicsRootDescriptorTable(
          Index, D3D12_GPU_DESCRIPTOR_HANDLE(reference));
      break;
    case RootSignatureUsage::Compute:
      allocator->SetComputeRootDescriptorTable(
          Index, D3D12_GPU_DESCRIPTOR_HANDLE(reference));
      break;
    default:
      throw winrt::hresult_not_implemented();
