...
void GraphicsPipelineStateDefinition::AddCachedPso(
    std::span<const uint8_t> buffer) {
  CachedPso = buffer;
}

GraphicsPipelineStateDefinition::operator D3D12_GRAPHICS_PIPELINE_STATE_DESC()
    const {
  D3D12_GRAPHICS_PIPELINE_STATE_DESC result{
      .pRootSignature = RootSignature->get(),
      .VS = VertexShader ? D3D12_SHADER_BYTECODE(*VertexShader)
                         : D3D12_SHADER_BYTECODE{nullptr, 0},
      .PS = PixelShader ? D3D12_SHADER_BYTECODE(*PixelShader)
                        : D3D12_SHADER_BYTECODE{nullptr, 0},
      .DS = DomainShader ? D3D12_SHADER_BYTECODE(*DomainShader)
                         : D3D12_SHADER_BYTECODE{nullptr, 0},
      .HS = HullShader ? D3D12_SHADER_BYTECODE(*HullShader)
                       : D3D12_SHADER_BYTECODE{nullptr, 0},
      .GS = GeometryShader ? D3D12_SHADER_BYTECODE(*GeometryShader)
                           : D3D12_SHADER_BYTECODE{nullptr, 0},
