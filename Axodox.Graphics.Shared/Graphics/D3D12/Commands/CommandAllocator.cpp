...
CommandAllocator::CommandAllocator(const GraphicsDevice &device,
                                   CommandKind type)
    : _device(device), _type(type) {
  check_hresult(device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE(type),
                                               IID_PPV_ARGS(_allocator.put())));
}
...
void CommandAllocator::BeginList(PipelineState *pipelineState) {
  if (_recorder)
    throw logic_error(
        "Cannot start a command list before finishing the last one!");

  _recorder = _lists.borrow();
  if (!_recorder->_list) {
    check_hresult(_device->CreateCommandList(
        0u, D3D12_COMMAND_LIST_TYPE(_type), _allocator.get(),
...
Collections::object_pool_handle<CommandList> CommandAllocator::EndList() {
  if (!_recorder)
    throw logic_error("Cannot end a command list before starting one!");
...
CommandKind CommandAllocator::Type() const { return _type; }

void CommandAllocator::AddAwaiter(CommandFenceMarker marker) {
  if (!_recorder)
    throw logic_error("There is no active command list!");
...
void CommandAllocator::TransitionResource(ResourceArgument resource,
                                          ResourceStates from,
                                          ResourceStates to) {
  D3D12_RESOURCE_BARRIER barrier{
      .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
      .Transition = {.pResource = resource.Pointer,
...
void CommandAllocator::AddUAVBarrier(ResourceArgument resource) {
  D3D12_RESOURCE_BARRIER barrier{.Type = D3D12_RESOURCE_BARRIER_TYPE_UAV,
                                 .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
                                 .UAV = {.pResource = resource.Pointer}};

  (*this)->ResourceBarrier(1, &barrier);
}

void CommandAllocator::TransitionResources(
    std::initializer_list<ResourceTransition> resources) {
...
void CommandAllocator::CopyResource(ResourceArgument source,
                                    ResourceArgument destination) {
  (*this)->CopyResource(destination.Pointer, source.Pointer);
}

void CommandAllocator::SetRenderTargets(
