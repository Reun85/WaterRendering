...
DescriptorHeap::DescriptorHeap(const GraphicsDevice &device,
                               DescriptorHeapKind type)
    : _device(device), _type(type), _handleBase{0} {}
...
DescriptorHeapKind DescriptorHeap::Type() const { return _type; }
...
void DescriptorHeap::Build() {
  ...
  if (!_heap || _heap->GetDesc().NumDescriptors < descriptorCount) {
    ...
    check_hresult(
        _device->CreateDescriptorHeap(&description, IID_PPV_ARGS(_heap.put())));
    _handleBase = _heap->GetCPUDescriptorHandleForHeapStart();
  }

  // Realize descriptors
  auto increment = _device->GetDescriptorHandleIncrementSize(
...
void DescriptorHeap::OnHeapBuilt(ID3D12DescriptorHeap * /*heap*/,
                                 uint32_t /*descriptorCount*/) {}

int64_t
DescriptorHeap::GetHandleOffset(D3D12_CPU_DESCRIPTOR_HANDLE handle) const {
