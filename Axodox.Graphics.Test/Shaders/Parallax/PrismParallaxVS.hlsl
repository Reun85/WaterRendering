...
{
    float4 t1 = heightMap1.SampleLevel(_sampler, GetTextureCoordFromPlaneCoordAndPatch(uv, debugValues.patchSizes.r), 3);
...
output_t main(input_t input)
{
    ...
    float4 height = readHeightMapWith(localPos.xz);

    localPos.y = localPos.y > eps ? height.y : -5;
